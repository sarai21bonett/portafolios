¡Hola a todos! Soy Sarai Bonett. aqui comparto un poco de mi experiencia en la informatica.

Perfil Profesional:
Técnica Superior Universitaria en Informática, actualmente Estudiante de Ingeniería en informática con experiencia en reparación de computadoras, 
robótica básica y jueza de robótica en la WRO (World Robot Olympiad).

Poseo habilidades en Python e inteligencia artificial, HTML, CSS, JavaScript y MySQL. Comprometida con el aprendizaje continuo y en busca de oportunidades
para aplicar y desarrollar mis habilidades técnicas.

🛠️ Tecnologías y Herramientas:
Lenguajes: Python, JavaScript, HTML, CSS, MySQL.
Especialidades: Inteligencia Artificial (curso IBM & Samsung), Robótica basica y Soporte Técnico.

🎯 Metas y Proyectos
Proyectos actuales: Desarrollo de sistemas web y auditoría de software.
Visión: Impactar en el área de la Robotica, biomecatrónica y crear soluciones tecnológicas innovadoras.
